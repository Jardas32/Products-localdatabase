...
function quantityPrices() {
    let total = storege.reduce((pre, item) => {
        return pre + parseFloat(item.prices.replace('$', '')) * item.quantity;
    },0)
...
function renderCard() {
    ...
    if(storege) {
        ...
        let {id, imgas, titles, prices, quantity = 1} = item;
        let price = parseInt(prices.replace('$', ''));
        let priceQuant = price * quantity;
        let pricestring = priceQuant.toLocaleString('en-US', {
            style: 'currency',
